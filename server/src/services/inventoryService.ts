...
export class InventoryService {
  ...
  async getAllInventory(
    ...
    SELECT
        ...
        COALESCE(SUM(sales.quantity), 0) AS total_sale_quantity,
        COALESCE(SUM(purchases.quantity), 0) - COALESCE(SUM(sales.quantity), 0) AS inventory_balance
    FROM
        product
    LEFT JOIN
        purchases ON product.id = purchases.product_id
    LEFT JOIN
        sales ON product.id = sales.product_id
    WHERE
